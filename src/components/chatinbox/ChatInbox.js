...
function ChatInbox({ senderUser, showProfInfo, setShowProfInfo, setHide }) {
  ...
  const answerCall = () => {
    ...
    peer.on("signal", (data) => {
      socket.emit("answerCall", {
        signal: data,
        to: selectedUser ? selectedUser.subUser._id : caller,
      });
    });
    peer.on("stream", (stream) => {
      userVideo.current.srcObject = stream;
  ...
  return (
    <>
        ...
        <div className="row chatib">
          <div className="col-sm-12 header_chat">
            <div className="display_header">
              <div className="conver2" onClick={() => setHide(true)}>
                <AiFillCaretRight />
              </div>
              {selectedUser && (
                <img
                  className="chat_icon"
                  src={senderUser ? senderUser.imageUrl : melissa}
                />
              )}
              <p className="online">
                {selectedUser && selectedUser.subUser?.email.split("@")[0]}
              </p>
            </div>
      ...
      <ImagModal imgd={imgd} setImgd={setImgd} sendi={sendi} />
    </>
  );
}
